...
async function main() {
  ...
  if (!supplierKey || !supplierProductId || !marketplaceKey || !marketplaceListingId) {
    console.error(
      "Usage: pnpm exec tsx scripts/approve_profitable_candidate.ts <supplier_key> <supplier_product_id> <marketplace_key> <marketplace_listing_id>"
    );
    process.exit(1);
  }
  ...
  const result = await db
    ...
    .set({
      ...
      reason: `approved manually | ${supplierKey} | ${supplierProductId} | ${marketplaceKey} | ${marketplaceListingId}`,
    })
    .where(
      and(
