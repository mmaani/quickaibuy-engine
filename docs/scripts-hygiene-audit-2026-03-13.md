...
RISK CLASSIFICATION:
- high risk scripts:
  - **DB mutation / state change**: `approve_profitable_candidate.ts`, `reject_profitable_candidate.ts`, `promote_listing_previews_ready.ts`, `promote_single_listing_ready.ts`, `prepare_one_ready_to_publish.ts`, `mutate_listings_mark_stale_publish_failed.ts`, `reclassify_legacy_dry_run_active_listings.ts`, `refresh_listing_price_guard_readiness.ts`, `fix_single_listing_payload_for_retry.ts`, `backfill_*`, `patch_marketplace_scan_summary_labels*.py`, `mutate_execute_sql_file.mjs`.
...
- Naming mismatch between behavior and perceived safety:
  - `mutate_git_push_pipeline_to_main.sh` performs `git add .`, commit, and push to `main` (very high-impact behavior behind generic “push pipeline” label).
  - `mutate_listings_mark_stale_publish_failed.ts` mutates listing states but could be mistaken for harmless cleanup.
  - `mutate_execute_sql_file.mjs` executes arbitrary SQL file content; generic name hides severity.
...
DANGEROUS SCRIPTS:
- `scripts/mutate_git_mutate_git_push_pipeline_to_main.sh`:
...
- Rename hazardous state changers for clarity:
  - `mutate_listings_mark_stale_publish_failed.ts` → `mutate_listings_mark_stale_publish_failed.ts`.
  - `mutate_execute_sql_file.mjs` → `mutate_execute_sql_file.mjs` (with guardrails).
...
3. Rename high-impact ambiguous scripts (`mutate_execute_sql_file.mjs`, `mutate_listings_mark_stale_publish_failed.ts`, `mutate_git_push_pipeline_to_main.sh`) to explicit-risk names.
