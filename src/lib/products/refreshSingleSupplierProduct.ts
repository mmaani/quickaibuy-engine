...
function parseSearchTextFromUrl(url: string | null | undefined): string | null {
  ...
  try {
    ...
    const query = parsed.searchParams.get("SearchText");
...
function deriveKeyword(row: NonNullable<LatestSupplierSnapshot>): string | null {
  ...
  const title = String(row.title ?? "").replace(/\s+sample from AliExpress$/i, "").trim();
  return title || null;
}
...
function buildAliExpressSyntheticRefreshRow(
  ...
  return {
    title: current.title ?? sample?.title ?? `${keyword} sample from AliExpress`,
    ...
    availabilityConfidence: parsedRows.length > 0 ? 0.2 : 0.12,
...
export async function refreshSingleSupplierProduct(input: {
  ...
  const refreshed = await refreshAliExpressSingleProduct(current, keyword);
