...
function classifyTemuFetchFailure(input: { mode: string; status: number; text: string }): {
  crawlStatus: string;
  ...
  if (/security verification|slide to complete the puzzle|challenge/i.test(text)) {
    return {
      crawlStatus: "CHALLENGE_PAGE",
      telemetrySignals: ["fallback", "challenge", "low_quality"],
      fetchError: "temu_security_challenge",
    };
  }
  return {
...
async function fetchTemuDetailText(detailUrl: string): Promise<{ text: string; mode: string; status: number }> {
...
async function fetchTemuSearchText(searchUrl: string): Promise<{ text: string; mode: string; status: number }> {
  const fetched = await fetchSupplierPageWithFallback({
    url: searchUrl,
    accept: "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
    validate: ({ text, status }) => status >= 200 && status < 300 && text.length > 500 && !looksLikeTemuChallengePage(text),
...
async function fetchTemuSearchFallbackText(searchUrl: string): Promise<{ text: string; mode: string; status: number }> {
  const fetched = await fetchSupplierPageWithFallback({
    url: searchUrl,
    accept: "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
    validate: ({ text, status }) => status >= 200 && status < 300 && text.length > 500 && !looksLikeTemuChallengePage(text),
...
export async function searchTemuByKeyword(
  ...
  try {
    ...
    const fetchFailure = classifyTemuFetchFailure({
