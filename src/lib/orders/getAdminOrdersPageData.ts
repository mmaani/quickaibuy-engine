  ...
  const [itemsRows, attemptsRows, readiness, eventsRows] = await Promise.all([
    db.execute<AdminOrderItem>(sql`
      SELECT
        ...
        NULL::text AS "linkageSource",
        NULL::text AS "linkageVerifiedAt",
        FALSE AS "linkageDeterministic",
        FALSE AS "supplierLinkLocked",
        NULL::text AS "supplierStockStatus",
        NULL::int AS "supplierStockQty",
        NULL::text AS "stockVerifiedAt",
        NULL::text AS "stockSource",
        TRUE AS "stockCheckRequired",
