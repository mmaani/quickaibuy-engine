...
export async function POST(request: Request) {
  ...
  const result = await resumePausedListing({
    listingId,
    actorType: "ADMIN",
  ...
  if (!result.ok) {
    redirectUrl.searchParams.set("resumeError", result.reason ?? "Unable to resume listing");
    return NextResponse.redirect(redirectUrl, { status: 303 });
  }

  redirectUrl.searchParams.set("resumeUpdated", "1");
  return NextResponse.redirect(redirectUrl, { status: 303 });
}
